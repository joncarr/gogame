...
func makeNoise(pixels []byte, frequency, lacunarity, gain float32, octaves int) {
	noiseArray := make([]float32, winWidth*winHeight)
	clearConsole()
	fmt.Printf("Frequency: %f\nLacunarity: %f\nGain: %f\nOctave: %d", frequency, lacunarity, gain, octaves)
	i := 0
	min := float32(9999.0)
	max := float32(-9999.0)
	...
	for y := 0; y < winHeight; y++ {
		for x := 0; x < winWidth; x++ {
			//noiseArray[i] = fbm2(float32(x), float32(y), frequency, lacunarity, gain, octaves)
			noiseArray[i] = turbulence(float32(x), float32(y), frequency, lacunarity, gain, octaves)
			if noiseArray[i] < min {
				min = noiseArray[i]
			} else if noiseArray[i] > max {
				max = noiseArray[i]
			}
			i++
		}
	...
	rescaleAndDraw(noiseArray, min, max, gradient, pixels)
}
...
func main() {
	...
	window, err := sdl.CreateWindow("NOISE!", sdl.WINDOWPOS_UNDEFINED, sdl.WINDOWPOS_UNDEFINED, int32(winWidth), int32(winHeight), sdl.WINDOW_SHOWN)
	if err != nil {
		panic(err)
	}
